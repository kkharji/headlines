...
//         // let x = eframe::egui::RichText::new($text);
...
//     // // )*
//     // o
//     // println!("{:?}", Vec::from$($args)*))};};};
// }

// macro_rules! Text {
//     ($text:expr, $($key:ident => $val:expr),* ,) => {
//         let mut map: std= std::collections::HashMap::new::<String, String>();
//     };
// }
macro_rules! __text_internal {
    (@array [$($elems:expr,)*]) => {};
    // Next element is an expression followed by comma.
    (@array [$($elems:expr,)*] $next:expr, $($rest:tt)*) => {
        __text_internal!(@array [$($elems,)* json_internal!($next),] $($rest)*)
    };

    // Last element is an expression with no trailing comma.
    (@array [$($elems:expr,)*] $last:expr) => {
        __text_internal!(@array [$($elems,)* json_internal!($last)])
    };
...
macro_rules! Text {
    ($($tokens:tt)+) => {{
        // __text_internal!($($tokens)+)
        // let mut m = eframe::egui::RichText::new($text);
        // $(
        //     m = m.$key($value);
        // )+
        // m
...
macro_rules! json_internal {
    ...
    ({ $($tt:tt)+ }) => {
        // $crate::Value::Object({
        {
            let mut object = eframe::egui::RichText::default();
